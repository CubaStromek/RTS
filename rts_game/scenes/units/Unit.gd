...
## Base unit with selection, movement, combat, effects, and placeholder visuals.
...
const DamageNumberScript := preload("res://scenes/effects/DamageNumber.gd")
const DeathEffectScript := preload("res://scenes/effects/DeathEffect.gd")
...
func _physics_process(delta: float) -> void:
	...
	if attack_target and is_instance_valid(attack_target):
		...
		_facing_angle = global_position.angle_to_point(attack_target.global_position)
...
func _draw() -> void:
	...
	# Unit body
	var base_color := FRIENDLY_COLOR if team == 0 else ENEMY_COLOR
	var color := Color.WHITE if _hit_flash_timer > 0.0 else base_color
	draw_circle(Vector2.ZERO, UNIT_RADIUS, color)

	# Facing direction indicator
	var facing_dir := Vector2(cos(_facing_angle), sin(_facing_angle))
	draw_line(Vector2.ZERO, facing_dir * UNIT_RADIUS * 0.8, base_color.darkened(0.4), 2.0)
...
func _draw_hp_bar() -> void:
	...
	if hp_ratio > 0.6:
		fill_color = Color(0.0, 0.8, 0.0)
	elif hp_ratio > 0.3:
		fill_color = Color(0.9, 0.7, 0.0)
	else:
		fill_color = Color(0.9, 0.2, 0.0)
...
func _perform_attack() -> void:
	...
	if attack_target and is_instance_valid(attack_target) and attack_target.has_method("take_damage"):
		...
		_spawn_attack_line(global_position, attack_target.global_position)
		var snd := get_node_or_null("/root/SoundSystem")
		if snd:
			snd.play("attack")
...
func _spawn_damage_number(amount: int) -> void:
	...
	dmg_node.set_script(DamageNumberScript)
	get_tree().current_scene.add_child(dmg_node)
	dmg_node.setup(amount, global_position)
...
func _spawn_death_effect() -> void:
	...
	effect.setup(global_position, color)
